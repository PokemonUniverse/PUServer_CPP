...
#define STARTPOINT_X -368
#define STARTPOINT_Y -101
...
public:
	...
	{
		if(std::abs((double)(p1.z - p2.z)) > deltaz || 
			std::abs((double)(p1.x - p2.x)) > deltax || 
			std::abs((double)(p1.y - p2.y)) > deltay)
		{
			return false;
		}
		return true;
	...
	inline static bool areInRange(const Position& p1, const Position& p2) 
	{
		if(std::abs((double)(p1.x - p2.x)) > deltax || 
			std::abs((double)(p1.y - p2.y)) > deltay) 
		{
			return false;
		}
	...
	Position() : x(STARTPOINT_X), y(STARTPOINT_Y), z(0) { };
